# mgeeky/Penetration-Testing-Tools (red-teaming/) — Authorization Context Analysis

> **Framework note (per `framework.md`):** this analysis is explicitly a boundary
> test of the Cortex model. The subject is a publicly-published red-team toolkit
> by an attributed security researcher. The analysis is in-bounds (exposing
> *structure*); it is not a deployment guide (which would be *application* and
> out-of-bounds). The analysis's purpose is to make the framework's authorization
> layer *visible* by examining a case where the skeleton is richly populated and
> the repository-level violation set is approximately empty.
...
- Repository: [`mgeeky/Penetration-Testing-Tools/tree/master/red-teaming`](https://github.com/mgeeky/Penetration-Testing-Tools/tree/master/red-teaming)
- Author: Mariusz Banach ("mgeeky"), `mb@binary-offensive.com`
- Cited prior work (non-exhaustive): harmj0y's GPO-permissions-abuse research, xpn's AppLocker CLM bypass via COM, Matt Graeber's AMSI tools, Casey Smith (subTee)'s MSBuild code-execution research, Stan Hegt's EvilClippy, F-Secure Labs' C3 framework, Mandiant's OfficePurge, PowerSploit / PowerView, GhostPack / SharpWMI
- Framework reference: `framework.md` §Scope Boundaries — "Is this analysis (exposing structure) or application (helping use it)?"
- Corpus cross-reference: compare hypothetical unauthorized-deployment analysis against `volt_typhoon.md` for the shared LOTL violation set
