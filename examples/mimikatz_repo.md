# gentilkiwi/mimikatz — Authorization Context Analysis

> **Framework note (per `framework.md`):** second boundary-test sample in the
> corpus, paired with `mgeeky_red_teaming_repo.md`. Unlike mgeeky's collection
> of diverse primitives, Mimikatz is a single-purpose tool with a narrow,
> high-consequence capability. The pairing sharpens a distinction within the
> null-violation class: *diffuse capability* vs. *concentrated capability*,
> both under externally-supplied authorization.
...
- Repository: [`gentilkiwi/mimikatz`](https://github.com/gentilkiwi/mimikatz) (created 2014-04-06)
- Author: Benjamin Delpy ("gentilkiwi"), `benjamin@gentilkiwi.com`, https://blog.gentilkiwi.com
- Project wiki: https://github.com/gentilkiwi/mimikatz/wiki
- Derivative reimplementations (notable): Invoke-Mimikatz (PowerShell, PowerSploit), Rubeus (C#, GhostPack), SharpDump (C#), PyKatz (Python), ProcDump + MemProcFS (live-memory extraction chain)
- Corpus cross-references: `mgeeky_red_teaming_repo.md` (paired boundary test, diffuse capability), `volt_typhoon.md` (same tool, different authorization context, confirmed state-actor use)
