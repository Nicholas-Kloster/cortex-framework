# Phishing / 419 Advance-Fee Scam (as a class) — Authorization Context Analysis

> **Framework note:** this sample is intentionally minimal in operations and
> maximal in claimed rights, to surface a *positive skeleton-to-violation gap*
> — a case where the attacker's claimed authorization dwarfs the technical
> action they perform. Chosen to test that the gap metric works at both tails
> of the distribution.
...
- FBI Internet Crime Complaint Center (IC3) annual reports — canonical quantitative documentation of scale and loss
- APWG (Anti-Phishing Working Group) quarterly reports
- UNODC "Cybercrime: an overview of incidents and issues" and successor publications — jurisdictional enforcement gaps
- Academic literature: Herley, "Why Do Nigerian Scammers Say They Are From Nigeria?" (Microsoft Research, WEIS 2012) — analysis of why attackers deliberately use implausible scripts to filter for high-response-likelihood targets
- Historical: "The Spanish Prisoner" confidence trick, documented in 19th-century press and predating electronic communication
- Corpus cross-references: `sunburst.md` (identity-layer forgery), `notpetya.md` (pretense violation), `llm_jailbreak_chains.md` (cognitive-surface targeting in the LLM context)
